...
```
AWS RDS MySQL (us-west-2)
  └─ Banco: JDOC_ANALYTICS
       ├─ d_organizations       ──────────────────────────────┐
       ├─ d_equipment           ──> d_equipments (modelo)     │
       ├─ d_devices             ──────────────────────────────┤
       ├─ d_machine_measurement ──────────────────────────────┤──> Power BI Desktop
       ├─ f_machine_alerts      ──────────────────────────────┤     └─> Medidas DAX
       ├─ f_machine_breadcrumbs ──────────────────────────────┤          └─> Dashboard
       ├─ f_machine_measurements ─────────────────────────────┤              (Power BI Service)
       └─ f_engine_hours        ──────────────────────────────┘

SharePoint (Excel)
  └─ Versões de Software.xlsx ──> d_versao_software (modelo)

Gerado em runtime (sem fonte)
  ├─ dCalendário (DAX CALENDAR)
  └─ _att_at (DateTimeZone.UtcNow)
```
...
**SLA sugerido:** até 2 dias úteis para concessão de acesso padrão.
...
| Passo | Ação |
|---|---|
| 1 | Identificar a medida exata que apresenta divergência (consultar o Dicionário de Métricas — task 5022) |
